...
mod.CollectibleType.COLLECTIBLE_SWEET_CAFFEINE = Isaac.GetItemIdByName("Sweet Caffeine")

local ENERGY_DRINK_CHANCE_BONUS = 15

function sweetCaffeine:onPickup(player)
...
mod:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, function(_, player)
    if player:HasCollectible(mod.CollectibleType.COLLECTIBLE_SWEET_CAFFEINE) then
        mod:onPickup(player)
    end
end)

function sweetCaffeine:OnGetCard(rng, card, decrease)
...
mod:AddCallback(ModCallbacks.MC_GET_CARD, sweetCaffeine.OnGetCard)
