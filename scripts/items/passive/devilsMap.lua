...
mod.CollectibleType.COLLECTIBLE_DEVILS_MAP = Isaac.GetItemIdByName("Devil's Map")
...
function devilsMap:OnNewRoom()
    local room = game:GetRoom()

    for i = 0, game:GetNumPlayers() - 1 do
        local player = Isaac.GetPlayer(i)
        if player:HasCollectible(mod.CollectibleType.COLLECTIBLE_DEVILS_MAP) then
            local data = player:GetData()
...
mod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, devilsMap.OnNewRoom)

-- duplicate in mod
function devilsMap:OnCache(player, cacheFlag)
    local data = player:GetData()
...
mod:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, devilsMap.OnCache)

function devilsMap:OnNewFloor()
    for i = 0, game:GetNumPlayers() - 1 do
        local player = Isaac.GetPlayer(i)
        local data = player:GetData()
        data.BoostedRooms = {}
...
mod:AddCallback(ModCallbacks.MC_POST_NEW_LEVEL, devilsMap.OnNewFloor)

function devilsMap:DevilsMapRender()
...
mod:AddCallback(ModCallbacks.MC_POST_RENDER, devilsMap.DevilsMapRender)
