...
mod.CollectibleType.COLLECTIBLE_BORROWED_STRENGTH = Isaac.GetItemIdByName("Borrowed Strength")

local BorrowedStrength = {}

function BorrowedStrength:onCache(player, cacheFlag)
    if not player:HasCollectible(mod.CollectibleType.COLLECTIBLE_BORROWED_STRENGTH) then return end
...
function BorrowedStrength:OnNewFloor()
    for i = 0, game:GetNumPlayers() - 1 do
        ...
        if player:HasCollectible(mod.CollectibleType.COLLECTIBLE_BORROWED_STRENGTH) then
