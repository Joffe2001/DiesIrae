...
SaveManager.Utility.JsonIncompatibilityType = {
    ...
    INVALID_KEY_TYPE =
    "Error at index \"%s\" with value \"%s\", type \"%s\": Tables that have non-string or non-integer (decimal or non-number) keys cannot be encoded.",
    MIXED_TABLES =
    "Index \"%s\" with value \"%s\", type \"%s\", found in table with initial type \"%s\": Tables with mixed key types cannot be encoded.",
    NAN_VALUE = "Tables with invalid numbers (NaN, -inf, inf) cannot be encoded.",
    INVALID_VALUE =
    "Error at index \"%s\" with value \"%s\", type \"%s\": Tables containing anything other than strings, numbers, booleans, or other tables cannot be encoded.",
...
function SaveManager.Utility.GetAscentSaveIndex()
    ...
    elseif game:GetRoom():GetType() == RoomType.ROOM_TREASURE or game:GetRoom():GetType() == RoomType.ROOM_BOSS then
        ...
        stageType = stageType >= StageType.STAGETYPE_REPENTANCE and StageType.STAGETYPE_REPENTANCE or
    ...
    for index, value in pairs(tab) do
        ...
        if type(index) ~= indexType then
            ...
            return SaveManager.Utility.ValidityState.INVALID,
                SaveManager.Utility.JsonIncompatibilityType.MIXED_TABLES:format(index, tostring(value), valType,
                    indexType)
        end
        ...
        elseif type(value) ~= "string" and type(value) ~= "boolean" then
            ...
            return SaveManager.Utility.ValidityState.INVALID,
                SaveManager.Utility.JsonIncompatibilityType.INVALID_VALUE:format(index, tostring(value), valType)
            --end
        end
    end
...
function SaveManager.Utility.AddDefaultRoomData()
    print(("[%s IsaacSaveManager] AddDefaultRoomData is deprecated! Please use AddDefaultTempData instead."):format(
    modReference.Name))
end
...
function SaveManager.Utility.GetPickupIndex(pickup)
    ...
    if myosotisCheck or movingBoxCheck then
        ...
        local targetTable = myosotisCheck and hourglassBackup[lastUsedHourglassSlot].pickupRoom or
        dataCache.game.movingBox
...
function SaveManager.Utility.GetPickupData(pickup)
    ...
    if not pickupData and game:GetLevel():IsAscent() then
        ...
        elseif roomType == RoomType.ROOM_TREASURE then
            pickupData = dataCache.game.treasureRoom[ascentIndex] and
            dataCache.game.treasureRoom[ascentIndex][pickupIndex]
        end
    end
        ...
        for _, originalPickup in ipairs(Isaac.FindByType(EntityType.ENTITY_PICKUP, dupedPickup.Variant)) do
            ...
            then
                ...
                if originalSaveData then
                    local result = Isaac.RunCallbackWithParam(SaveManager.SaveCallbacks.DUPE_PICKUP_DATA_LOAD,
                        originalPickup.Variant, originalPickup, dupedPickup, originalSaveData)
                    if not result then
...
local function storeAndPopulateAscent()
    ...
    if not game:GetLevel():IsAscent() then
        ...
        if roomSaveData and roomSaveData.__SAVEMANAGER_ASCENT_ROOM_TYPE then
            ...
            local targetTable = roomType == RoomType.ROOM_TREASURE and dataCache.game.treasureRoom or
            dataCache.game.bossRoom
            local ascentIndex = SaveManager.Utility.GetAscentSaveIndex()
            if not ascentIndex then return end
    ...
    then
        ...
        local targetTable = currentRoomDesc.Data.Type == RoomType.ROOM_TREASURE and dataCache.game.treasureRoom or
        dataCache.game.bossRoom
        local ascentIndex = SaveManager.Utility.GetAscentSaveIndex()
        if not ascentIndex then return end
        local ascentRoomData = targetTable[ascentIndex]
...
local function onEntityInit(_, ent)
    ...
    if player and isLazB[player:GetPlayerType()] then
        SaveManager.Utility.DebugLog(player:GetPlayerType(), player:GetCollectibleRNG(1):GetSeed(),
            player:GetCollectibleRNG(2):GetSeed(), defaultSaveIndex)
    end
    ...
    local function resetNoRerollData(targetTable, defaultTable, checkIndex)
        ...
        if data and ent and data.InitSeed and data.InitSeed ~= ent.InitSeed then
            Isaac.RunCallbackWithParam(SaveManager.SaveCallbacks.PRE_PICKUP_INITSEED_MORPH, ent.Variant, ent,
                data.NoRerollSave)
            if data.InitSeedBackup and ent.InitSeed == data.InitSeedBackup then
                ...
                Isaac.RunCallbackWithParam(SaveManager.SaveCallbacks.POST_PICKUP_INITSEED_MORPH, ent.Variant, ent,
                    data.NoRerollSave)
                return
            end
            data.NoRerollSaveBackup = SaveManager.Utility.DeepCopy(data.NoRerollSave)
...
function SaveManager.InitMinimapAPI(minimapAPI, branchVersion)
    ...
    if minimapAPI.BranchVersion == branchVersion then
        ...
        modReference:AddPriorityCallback(ModCallbacks.MC_POST_GAME_STARTED,
            SaveManager.Utility.CallbackPriority.IMPORTANT, function(_, isContinue)
        ...
        modReference:AddPriorityCallback(ModCallbacks.MC_PRE_GAME_EXIT, SaveManager.Utility.CallbackPriority.LATE - 1,
