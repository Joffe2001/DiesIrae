...
mod:AddCallback(ModCallbacks.MC_POST_NPC_INIT, function(_, npc)
    ...
    if variant == mod.CHALLENGES.NO_HIT_CHAMPIONS then
        local config = Isaac.GetEntityConfig():GetEntity(npc.Type, npc.Variant, npc.SubType)
        if config and config:CanBeChampion() and not npc:IsChampion() and math.random() < 0.15 then
