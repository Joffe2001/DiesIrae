...
mod.RegisteredElijahBeggars = mod.RegisteredElijahBeggars or {}

function mod:RegisterElijahBeggar(registration)
    if type(registration) ~= "table" then
        print("[Dies Irae API] RegisterElijahBeggar ERROR: registration must be a table.")
        return
    end
    if not registration.variant or type(registration.variant) ~= "number" then
        print("[Dies Irae API] RegisterElijahBeggar ERROR: 'variant' must be a number.")
        return
    end
    if not registration.config or type(registration.config) ~= "table" then
        print("[Dies Irae API] RegisterElijahBeggar ERROR: 'config' must be a table.")
        return
    end
    if not registration.primaryEvents or type(registration.primaryEvents) ~= "table" then
        print("[Dies Irae API] RegisterElijahBeggar ERROR: 'primaryEvents' must be a table.")
        return
    end

    local variant = registration.variant

    if mod.RegisteredElijahBeggars[variant] then
        print("[Dies Irae API] RegisterElijahBeggar WARNING: variant " .. variant .. " already registered. Overwriting.")
    end

    mod.RegisteredElijahBeggars[variant] = {
        config          = registration.config,
        primaryEvents   = registration.primaryEvents,
        secondaryEvents = registration.secondaryEvents or nil,
    }

    mod:AddCallback(ModCallbacks.MC_POST_SLOT_COLLISION, function(_, beggarEntity, collider)
        local player = collider:ToPlayer()
        if not player then return end

        local ok, err = pcall(function()
            local paid = beggarFuncs.OnBeggarCollision(beggarEntity, player, registration.config)
            if paid then
        ...
        if not ok then
            print("[Dies Irae API] Beggar collision error (variant " .. variant .. "): " .. tostring(err))
            beggarFuncs.DoBeggarExplosion(beggarEntity)
        end
    end, variant)

    mod:AddCallback(ModCallbacks.MC_POST_SLOT_UPDATE, function(_, beggarEntity)
        local ok, err = pcall(function()
            beggarFuncs.StateMachine(
                beggarEntity,
                registration.config,
                registration.primaryEvents,
                registration.secondaryEvents
            )
        end)

        if not ok then
            print("[Dies Irae API] Beggar update error (variant " .. variant .. "): " .. tostring(err))
            beggarFuncs.DoBeggarExplosion(beggarEntity)
        end
    end, variant)

    mod:AddCallback(ModCallbacks.MC_PRE_SLOT_CREATE_EXPLOSION_DROPS, function(_, beggarEntity)
        local ok, err = pcall(function()
            beggarFuncs.DoBeggarExplosion(beggarEntity)
        end)
        if not ok then
            print("[Dies Irae API] Beggar explosion error (variant " .. variant .. "): " .. tostring(err))
            beggarEntity:Remove()
        end
        return false
    end, variant)

    print("[Dies Irae API] Registered Elijah beggar variant " .. variant)
end
